...
function getUserById(array, id) {
  console.log(array.find((obj) => obj.id === id))
}
...
function deleteUser(array, id) {
  let newArray = array.filter((obj) => obj.id !== id) // обьект не мутирует
  return newArray
}
...
function getUsersGeo(array, id) {
  let findedObj = array.find((obj) => obj.id === id)
    console.log(findedObj.address.geo)
}
...
function getUsersCompany(array, id) {
  let findObj = array.find((obj) => obj.id === id)
    console.log(findObj.company)
}

getUsersCompany(users, 4)
...
function changePhone(array, id, phone) {
  let newArray = array.find(function(obj){
    if(obj.id === id){
...
console.log(changePhone(users, 10, "+ 7 987 654 32 10"))
